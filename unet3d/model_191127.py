...
class UNet3D(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, f_maps=64, layer_order='crg', num_groups=8,
        ...
        # for inter-loss
        self.mid_conv0 = nn.Conv3d(f_maps[3], out_channels, 1)
        self.mid_conv1 = nn.Conv3d(f_maps[2], out_channels, 1)
        self.mid_conv2 = nn.Conv3d(f_maps[1], out_channels, 1)
        
        self.mid_convs = [self.mid_conv0, self.mid_conv1, self.mid_conv2]
        # self.mid_convs = []
        # self.mid_convs.append(nn.Conv3d(f_maps[3], out_channels, 1))
        # self.mid_convs.append(nn.Conv3d(f_maps[2], out_channels, 1))
        # self.mid_convs.append(nn.Conv3d(f_maps[1], out_channels, 1))
    ...
    def forward(self, x):
        ...
        for decoder, encoder_features in zip(self.decoders, encoders_features):
            ...
            # mid_feature = self.mid_convs[i](x)
            # loss_features.insert(0, mid_feature)
            # i += 1
            decoder_feature.append(x)
            x = decoder(encoder_features, x)
            # append each features

        loss_features.insert(0, self.mid_conv0(decoder_feature[0]))
        loss_features.insert(0, self.mid_conv1(decoder_feature[1]))
        loss_features.insert(0, self.mid_conv2(decoder_feature[2]))
...
def get_model(config):
    def _model_class(class_name):
        m = importlib.import_module('unet3d.model_191127')
