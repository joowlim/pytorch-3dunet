...
class UNet3DTrainer:
    ...
    def _forward_pass(self, input, target, weight=None):
        ...
        output, mid_features = self.model(input)
        ...
        if weight is None:
            ...
            #print('!')
            mid_losses = []
            target_reduce = target
            for i in range(3):
                mid = mid_features[i]
                
                target_reduce = target_reduce.double()
                target_reduce.unsqueeze_(0)
                target_reduce = F.interpolate(target_reduce, scale_factor = 0.5)
                target_reduce.squeeze_(0)
                target_reduce = target_reduce.long()
                loss += self.loss_criterion(mid, target_reduce)
